...
Operation staging/recovery files are internal only when a durable local-mutation operation explicitly owns their exact physical path. Running/recovering/blocked local operations may own their operation-ID-specific recovery slot (and `EnsureLocal` download slot). There is one narrower pre-side-effect case: a `planned`, `attempts=0` `EnsureLocal` with a complete durable local pin may own only its exact `download` staging slot, because a hard crash can leave that disposable file after download but before the coordinator crosses the `running` boundary; it does not thereby own a recovery slot. A filename that merely resembles `.pkudisk-sync-tmp-op-<id>-download` or `...-recovery`, or a planned operation with an incomplete pin / prior attempt, is not silently hidden and remains a reserved-namespace error. The remote scanner rejects the same reserved namespace, keeping local and remote authority symmetric.
