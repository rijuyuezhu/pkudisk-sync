...
One daemon can own multiple independent roots. Local roots may not overlap each other, and selected remote paths may not overlap. Complete scans keep physical **directory** ownership exclusive outside a `copy` projection, while ordinary regular-file hard links inside one root remain independent lexical entries; hard-link topology itself is not synchronized metadata. Every `follow` symlink target (file or directory) is additionally reserved in shared SQLite authority before planning or mutation: the same current physical identity or canonical physical target pathname cannot be owned by two configured roots, and a followed file cannot share its physical identity with an ordinary lexical file entry in the same scan. `copy` deliberately allows duplicate aliases and aliases back into the same selected root as independent lexical projections and creates no global claim, but it still fails closed when a resolved target overlaps another configured local root or crosses a foreign root marker. Ordinary cross-root bind-mount or hard-link overlap that does not pass through these symlink policies remains unsupported rather than globally enumerated. v0.1 intentionally uses one app-owned PKU Disk authentication profile named `pkudisk`; a config alias is not treated as account identity.
...
`copy` uses the same lexical namespace and source-byte projection as rclone `--copy-links`, but it intentionally does not acquire global physical ownership. A complete scan records non-owning `CopyProjectionEvidence` for every copied symlink boundary: the logical boundary path, entry kind, current physical object identity, and canonical target pathname. That evidence belongs only to the scan that produced the reconciliation decisions; it is not inserted into the global followed-ownership table. A final copied symlink is pinned with `lexical` authority: a local delete removes only that symlink object, and a remote file update materializes a regular file at the lexical pathname. A descendant reached through a copied directory symlink is pinned with `copy-physical` authority to the exact resolved referent destination for that operation. Immediately before pinning, the resolver produces the current boundary evidence again and the syncer requires exact continuity for every copied boundary traversed by that logical path. If a boundary disappeared, appeared, or retargeted since the complete scan, the unattempted operation is discarded and a fresh complete scan/replan is required even when the projected file fingerprint is unchanged. An unpinned copy-mode local operation that survives into a later cycle is likewise discarded because the scan evidence that authorized its original decision is no longer available. Duplicate/internal copy projections remain valid because this evidence is proof of scan-to-pin continuity, not a durable ownership claim.

The journal therefore distinguishes `lexical`, `follow-physical`, and `copy-physical` local authority. Before the **first** user-data side effect, the executor still revalidates the current logical path against the newly chosen pin, so a pin-to-side-effect retarget fails closed. Mutation-target continuity is pathname continuity rather than inode equality: the parent pathname is canonicalized to tolerate native aliases such as `/var` versus `/private/var` and Windows short-name spellings, but the leaf basename remains part of the authority, so two distinct hard-link pathnames never become interchangeable merely because `os.SameFile` is true. Physical object identity is checked separately by the pinned anchor. A final `copy` symlink is also resolved at pin time for fencing only: its referent must still be available, supported, non-cyclic, outside peer roots, and outside foreign root markers, even though the mutation target itself remains the lexical symlink object. The coordinator then durably enters `running` at the side-effect boundary. Once that boundary has been crossed, the journaled destination becomes the only recovery authority: postcondition observation, safe-replay preconditions, recovery content comparison, and replay use the pinned physical path/anchor and do not re-resolve the current alias. This prevents a concurrent `copy` directory retarget from redirecting an unknown-outcome recovery to a different referent. A later complete scan observes the retarget normally after the prior journal is resolved.

Hierarchical authority checks are serialized under SQLite writer authority before their read-check-write sequence. For `follow`, scan-time claim reservation, mutation-time claim refresh/pin, and sync-root creation therefore cannot concurrently observe an unclaimed parent/child namespace and both commit ownership. `copy` does not gain root-lifetime ownership, but a successfully journaled `copy-physical` pin is an operation-lifetime conflict authority: the pin transaction rechecks currently configured roots before committing, while `PrepareSyncRootCreate` checks all still-journaled `copy-physical` pins before reserving a new root. Parent/descendant pathname overlap is symmetric, so root-create versus copy-pin has exactly one winner under concurrent writers. The temporary fence disappears when the operation is committed/deleted; no followed-style global claim remains. Exact-identity/target triggers remain defense-in-depth for equality conflicts; subtree conflicts are enforced by the serialized authority transaction.
...
Outside a `copy` projection, complete scans keep physical directory identities exclusive so a followed expansion cannot acquire a directory/subtree already reached through another logical path. Ordinary regular-file hard links are different: each pathname is an independent lexical sync entry even when several names currently share one inode/file ID. Deleting one entry unlinks only that pathname, and a remote materialization/update may break the local hard-link relationship through the normal atomic replacement path; preserving hard-link topology is intentionally out of scope. A `follow` file symlink is still exclusive physical authority, so its referent identity may not already be exposed by an ordinary lexical file pathname and two followed file aliases may not share one referent. Inside a copy-projection subtree duplicate and internal aliases remain independent lexical projections as before. Followed symlink targets retain the additional daemon/store-wide subtree claim described above, so two roots cannot independently follow a parent/descendant pair such as `/shared` and `/shared/subdir` or `/shared/file.txt`. Copy projections still may not cross into another configured root. The implementation does not globally enumerate arbitrary non-symlink bind-mount/hard-link overlap between separately selected roots.

Operation staging/recovery files are internal only when a durable local-mutation operation explicitly owns their exact physical path. Running/recovering/blocked local operations may own their operation-ID-specific recovery slot (and file `EnsureLocal` download slot). There is one narrower pre-side-effect case: a `planned`, `attempts=0` file `EnsureLocal` with a complete durable local pin may own only its exact `download` staging slot, because a hard crash can leave that disposable file after download but before the coordinator crosses the `running` boundary; it does not thereby own a recovery slot. On restart, recovery removes that exact disposable download slot immediately after re-establishing the durable pin and before any stale-precondition check or known-unstarted discard, so deleting the operation row cannot orphan a reserved artifact that wedges the next complete scan. A filename that merely resembles `.pkudisk-sync-tmp-op-<id>-download` or `...-recovery`, a directory `EnsureLocal`, or a planned operation with an incomplete pin / prior attempt is not silently hidden and remains a reserved-namespace error. The remote scanner rejects the same reserved namespace, keeping local and remote authority symmetric.
